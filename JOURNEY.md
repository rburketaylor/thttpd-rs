...
The server is not described as a full operational drop-in replacement. CGI resource controls, the CGI working directory, and the optional `VHOST_DIRLEVELS` directory splitting are not yet at parity, and each is recorded with its legacy behavior, current Rust behavior, impact, and disposition in `docs/RISKS.md`. An explicit list of known gaps is more useful than an unqualified compatibility claim; it is what allows the 105 passing scenarios to mean what they appear to mean.
